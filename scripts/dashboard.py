...
def project(slug: str) -> dict:
    ...
    return {
        ...
        # Safe to expose: it contains workflow state only, never CMS/API secrets.
        "content_schedule": cfg.get("content_schedule") or {},
    }
...
def workbench(slug: str, qid: str) -> dict:
    ...
    import content_registry
    registry = content_registry.build(slug)
    rec = next((x for x in registry["records"] if x["question_id"] == qid), None)
    return {"question": q, "sources": sources, "content_status": rec}
...
class Handler(BaseHTTPRequestHandler):
    ...
    def do_POST(self):
        ...
        try:
            ...
            if p.startswith("/api/clean-global-questions/"):
                import bootstrap
                slug = p[len("/api/clean-global-questions/"):]
                return self._json(bootstrap.clean_global_questions(slug))
...
def _monitor_tick():
    ...
    for d in (G.WORK.iterdir() if G.WORK.exists() else []):
        ...
        if every and (mon.get("next_run") or "") <= G.today() and not J.running_for(d.name):
            try:
        ...
        # Content automation is separately opt-in.  It only creates CMS drafts
        # and uses last_run to guarantee at most one batch per local calendar day.
        content = cfg.get("content_schedule") or {}
        if not content.get("enabled") or content.get("last_run") == G.today():
            continue
        run_at = str(content.get("time") or "09:00")
        if not re.fullmatch(r"(?:[01]\d|2[0-3]):[0-5]\d", run_at):
            G.info(f"每日内容排程跳过 {d.name}：时间格式无效 {run_at!r}")
            continue
        if time.strftime("%H:%M") < run_at or J.running_for(d.name):
            continue
        try:
            J.start(d.name, "daily-content", {"--limit": int(content.get("daily_count") or 1)})
            content["last_run"] = G.today()
            content["last_result"] = "started"
            cfg["content_schedule"] = content
            G.save_config(d.name, cfg)
            G.info(f"每日内容排程触发：{d.name}，数量 {content.get('daily_count', 1)}")
        except (ValueError, RuntimeError) as e:
            G.info(f"每日内容排程跳过 {d.name}：{e}")
